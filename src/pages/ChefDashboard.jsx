...
const GlobalStyles = () => (
  <style>{`
    ...
    .spark {
      ...
      background-color: #fbbF24; border-radius: 50%;
    ...
    >
      ...
      >
        ...
        <p className="text-center text-xl font-mono text-orange-600 mb-4">
          #{order.billNumber ? order.billNumber.substring(5) : "N/A"}
        </p>

        <div className="mb-4 space-y-1">
          <p className="text-slate-700 text-lg">
            <span className="font-semibold">Customer:</span>{" "}
            {order.studentName || (order.student ? order.student.name : "N/A")}
          </p>
...
const OrderCard = ({ order, updateStatus, loading, onViewBill }) => {
  ...
  return (
    <div className="bg-slate-800 shadow-lg rounded-lg p-4 flex flex-col justify-between h-full transform transition duration-300 hover:scale-[1.02] hover:shadow-orange-500/30 border-l-4 border-red-500">
      <div>
        <div onClick={onViewBill} className="cursor-pointer">
          <div className="flex justify-between items-start mb-2">
            <h3 className="text-lg font-bold text-slate-100">
              BILL{" "}
              <span className="text-orange-400">
                #{orderIdentifier ? orderIdentifier.substring(5) : "N/A"}
              </span>
            </h3>
          ...
          <div className="p-2 bg-slate-700/50 rounded-md border border-slate-600">
            <p className="font-bold text-sm text-orange-300 mb-1">
            ...
            <ul className="text-xs text-slate-200 space-y-0.5 max-h-24 overflow-y-auto scrollbar-thin scrollbar-thumb-slate-600 scrollbar-track-slate-700 pr-1">
              {order.items?.map((item, index) => (
                <li key={index} className="flex justify-between">
                  ...
                  <span className="font-extrabold text-orange-400 flex-shrink-0">
                    x{item.quantity}
                  </span>
                </li>
              ))}
...
export default function ChefDashboard({ handleLogout }) {
  ...
  const handleMarkReady = useCallback(async (billNumber) => {
    try {
      ...
      console.error("Mark Ready Error:", serverMessage);
  ...
  if (error && !error.includes("Session expired")) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-slate-900 text-white p-6">
        ...
        <button
          onClick={loadOrders}
          className="mt-4 text-orange-400 hover:underline flex items-center"
        >
    ...
    <div className="min-h-screen bg-slate-900 font-sans p-4 sm:p-6 relative">
      ...
      <div className="relative z-10">
        ...
        <div className="max-w-xl mx-auto space-y-4">
          ...
          {!loading && orders.length === 0 && (
            <p className="text-slate-400 text-center text-lg p-10 mt-6 bg-slate-800/50 rounded-lg">
              No new bills to prepare right now.
            </p>
          )}
        </div>
